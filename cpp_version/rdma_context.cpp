...
void rdma_server_context::receive_file()  {
    ...
    char* file_buf = (char*)malloc(req->length);
    /* register a memory region for the input / output images. */
    mr_file = ibv_reg_mr(pd, file_buf, req->length, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
    if (!mr_file) {
        perror("ibv_reg_mr() on server failed for file");
        exit(1);
    }
...
bool rdma_client_context::send_file(int file_id, std::string filename)  {
    ...
    char *file_in_memory = (char*) mmap(NULL, file_size, PROT_READ, MAP_PRIVATE, fd, 0);

    struct ibv_mr *mr_file = ibv_reg_mr(pd, file_in_memory, file_size, IBV_ACCESS_REMOTE_READ);
    if (!mr_file) {
    ...
    req.length = file_size;
    req.addr = (uint64_t) file_in_memory;
